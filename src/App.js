...
import { Sun, Moon, Volume2, VolumeX, Send, Image, Trash2, Info, List, X, PlusCircle, Shuffle, Globe, AlertTriangle, Edit, RefreshCcw, FileText, Settings, Save, RotateCcw, Copy, Trash, Radio, File, CheckCircle } from 'lucide-react'; // Add Copy icon and File
...
// --- Styled Components --- (No major changes, just ensuring consistency)
...
const ChatArea = styled.div`
  ...
  padding-bottom: 80px; /* Add padding to account for the fixed input area */
  display: flex;
  flex-direction: column;
  gap: 1rem;
  z-index: 10;
...
// InputArea - ADDED a container for the attachment indicators
   ...
   const selectChat = (chatId) => {
    ...
    const preparePrompt = (userMessageContent, image = null, fileContent = null, isRegeneration = false) => {
        ...
        if (image) {
            ...
            // File content handling (still using gemini-pro)
            modelToUse = googleGenAI.getGenerativeModel({ model: "gemini-2.0-pro-exp-02-05" }); // Corrected model for file content
            prompt = language === 'vi'
                ? `Đây là nội dung từ một file và yêu cầu, hãy xử lý và trả lời bằng tiếng Việt:\n\n${userMessageContent}\n\n${fileContent}`
                        ...
                        <AttachmentIndicator darkMode={darkMode}>
                            {uploadedFile && (
                                <>
                                    ...
                                    <span>{language === 'vi' ? 'Tệp' : 'File'}</span>
